...
The selection protocol is frozen in ``exploratory/causal/SELECTION-RULE.md``.
...
RULE_PATH = PROJECT_ROOT / "exploratory" / "causal" / "SELECTION-RULE.md"
DEFAULT_REPO = PROJECT_ROOT / "exploratory" / "causal" / "repositories" / "pallets-flask"
DEFAULT_LEDGER = PROJECT_ROOT / "exploratory" / "causal" / "inventory" / "flask-candidates.json"
DEFAULT_RESULTS = PROJECT_ROOT / "exploratory" / "causal" / "verification" / "flask-results.json"
DEFAULT_FIXTURE = PROJECT_ROOT / "fixture" / "flask"
HEAD = "d318b683471101618febed18996405ad26462110"
CUTOFF_EPOCH = 1_577_836_800  # 2020-01-01T00:00:00Z
SQUASH_PR_RE = re.compile(r"\(#([1-9][0-9]*)\)$")
MERGE_PR_RE = re.compile(r"^Merge pull request #([1-9][0-9]*) ")
ALLOWED_ROOT_FILES = {"CHANGES.rst", "README.md"}
...
def inventory(repository: Path, output: Path) -> dict[str, Any]:
    ...
    for line in log.splitlines():
        if not line:
            continue
        sha, parents, timestamp, committed_at, subject = line.split("\t", 4)
        records.append(
            {
                "sha": sha,
    ...
    for position, record in enumerate(records, 1):
        ...
        squash_match = SQUASH_PR_RE.search(record["subject"])
        merge_match = MERGE_PR_RE.search(record["subject"])
        if len(parents) in {1, 2} and squash_match is not None:
            match = squash_match
            pr_pattern = f"{len(parents)}-parent PR-title subject suffix"
        elif len(parents) == 2 and merge_match is not None:
            match = merge_match
            pr_pattern = "two-parent merge subject prefix"
        else:
        ...
        if not (2 <= len(changes) <= 12):
            rejection_counts["path_count_outside_2_12"] += 1
            continue
        if any(change["status"] not in {"A", "M"} for change in changes):
            rejection_counts["status_not_A_or_M"] += 1
            continue
        commit_tree = ls_tree(repository, record["sha"])
        paths = [change["path"] for change in changes]
        if any(
            path not in commit_tree
            or commit_tree[path]["type"] != "blob"
            or commit_tree[path]["mode"] not in REGULAR_MODES
            for path in paths
        ):
            rejection_counts["changed_path_not_regular"] += 1
            continue
        source_paths = [
            path for path in paths if path.startswith("src/flask/") and path.endswith(".py")
        ]
        python_test_paths = [
        ...
        if any(
            ...
            rejection_counts["path_outside_frozen_roots"] += 1
            continue
        changed_test_paths = [path for path in paths if path.startswith("tests/")]
        if len(changed_test_paths) > 4:
            rejection_counts["more_than_four_test_paths"] += 1
            continue
        patch = full_patch(repository, parent, record["sha"])
        if len(patch) > 100 * 1024:
            rejection_counts["patch_over_100_KiB"] += 1
            continue
        if pr in seen_prs:
        ...
        eligible.append(
            {
                ...
                "pr_url": f"https://github.com/pallets/flask/pull/{pr}",
    ...
    value = {
        ...
        "counts": {
            ...
            "single_parent_squash_pr_landings": pr_landings,
...
def safe_extract_archive(repository: Path, revision: str, destination: Path) -> None:
    # Git for Windows' system config enables core.autocrlf. `git archive`
    # unexpectedly applies that worktree conversion unless it is overridden,
    # which would make every exported blob differ from its historical object.
    archive = git(
        repository,
        "-c",
        "core.autocrlf=false",
...
def verify_candidate(
    *,
    ...
    temporary = Path(tempfile.mkdtemp(prefix=f"f{pr}-", dir=temporary_root))
    try:
        base = temporary / "b"
...
def verify(
    ...
    *,
    ...
    evidence_root = output.parent / "arms"
    # Arms must live outside PROJECT_ROOT. Otherwise `git apply` discovers the
    # Blast-Radius parent repository and silently treats Flask's patch paths as
    # outside the current subdirectory instead of editing the arm.
    temporary_root = Path(tempfile.gettempdir()) / "blast-radius-flask-causal"
    ...
    try:
        ...
        manifest = {
            ...
            "patch_semantics": {
                ...
                "pr-N.tests.patch": "tests/ path subset only",
            },
